...
### Simple Example
The following command runs a single pass of encryption over the file `Constitution.pdf` using the key`fookeybar`. It uses variable-dimension encryption since the `D` flag was not invoked and outputs a file named `Constitution.pdf.fbz`.  

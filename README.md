...
Orthogonal matrices to the rescue! An orthogonal matrix is one whose transpose is also its inverse. Calculating the transpose of a square matrix has a time complexity of O(n), a vast improvement over matrix inverse which cannot be done in less than O(n<sup>2</sup>) time[<sup>4</sup>](#endnotes). Furthermore, all permutation matrices are orthogonal, meaning an algorithm that can efficiently generate unique permutation matrices from encryption keys would solve both problems.  
